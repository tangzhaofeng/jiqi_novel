...
function update_article($articleid,$articlename,$chapterorder,$chaptertype,$chapterinfo) {
    global $db;
...
//    return;
    ...
    $chaptername = addslashes(iconv("utf-8", "gbk", trim($chapterinfo['title'])));
    ...
    $sql="select * from jieqi_article_chapter where articleid=$articleid and chaptername='".$chaptername."'";
    $r=$db->query($sql);
    $d=$db->fetchArray($r);
    if ($d) {
        $chapterid=$d['chapterid'];
        $sql="update jieqi_article_chapter set lastupdate=$now,size=$size,saleprice=$price where chapterid=$chapterid";
    }
    else {
        $sql = "insert into jieqi_article_chapter (articleid,articlename,poster,postdate,lastupdate,chaptername,chapterorder,size,saleprice,isvip,chaptertype,display)";
        $sql .= " values('$articleid','$articlename','admin',$now,$now,'$chaptername',$chapterorder,$size,$price,$isvip,$chaptertype,0)";
    }
    ...
    if ($db->query($sql)) {
        ...
        if ($chapterorder==354) {
            echo iconv("GB18030","utf-8",$chaptercontent)."\n";
        }
...
//                            if ($chaptername == $last_chaptername) {
...
//                    echo $chapterinfo['content']."\n";
//                    echo "--------------------------------\n";
//                    echo format_txt($chapterinfo['content'])."\n";

                    update_article($articleid,$articlename,$chapterorder,0,$chapterinfo);
                    //print_r($chapterinfo);

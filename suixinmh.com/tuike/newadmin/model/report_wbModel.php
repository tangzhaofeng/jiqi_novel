 ...
 */ 
class report_wbModel extends Model{
  //login form
  ...
  public function mainList($params=array(),$custompage=JIEQI_PAGE_TAG,$emptyonepage = false){
    ...
    $sql ='SELECT q.statime,q.endtime,q.pdate,q.qd,q.wxh,q.wxn,q.fans,q.fee,q.articlename,q.read,o.company,o.feetype,o.feelence FROM '.
      jieqi_dbprefix('tuike_qdlistwb').' q '.
    ...
    while($row=$this->db->getRow($sqlres)){
      ...
      $sql='SELECT sum(click) as click,sum(pv) pv FROM '.
        jieqi_dbprefix('system_qddata').' WHERE ( qd="'.$row['qd'].'" '.$timeW.' ) LIMIT 1';
      $tAr=$this->db->getRow($this->db->query($sql));


      $row['click']=$tAr['click']?$tAr['click']:0;
      $row['pv']=$tAr['pv']?$tAr['pv']:0;
      ...
      $sql='SELECT count(*) FROM '.jieqi_dbprefix('system_users').' WHERE ( source="'.$row['qd'].'" '.$timeW.' )';
      ...
      $timeW=' AND rettime>="'.$row['statime'].'" AND rettime<="'.($row['endtime']>$t2?$t2:$row['endtime']).'" ';
      $sql='SELECT sum(round(money/100,2)) FROM '.jieqi_dbprefix('pay_paylog'). 
          ' WHERE ( payflag="1" AND source="'.$row['qd'].'" '.$timeW.' )';
  ...
  public function downQdloadList($params){
    ...
    $sql ='SELECT  q.statime,q.endtime,q.pdate,q.qd,q.wxh,q.wxn,q.fans,q.fee,q.articlename,q.read,sum(d.click) as click,sum(d.pv) pv,o.company,o.feetype,o.feelence FROM '.jieqi_dbprefix('tuike_qdlistwb').' q '.
      'LEFT JOIN '.jieqi_dbprefix('tuike_orderqd').' o ON o.id=q.orderqdid '.
    ...
    while($row=$this->db->getRow($sqlres)){
      ...
      /* 计算单价 */
      if( $row['feetype'] === '0' ){
        $row['uprice']=round($row['fee']/$row['fans'],2);
      ...
      $timeW=' AND rettime>="'.$row['statime'].'" AND rettime<="'.($row['endtime']>$t2?$t2:$row['endtime']).'" ';
      $sql='SELECT sum(round(money/100,2)) FROM '.jieqi_dbprefix('pay_paylog'). 
          ' WHERE ( payflag="1" AND source="'.$row['qd'].'" AND rettime<="'.$t2.'" '.$timeW.' )';
      $row['payallTh']=$this->db->getField($this->db->query($sql));
      $row['huiThIx']=$row['fee']==0?0:round($row['payallTh']/$row['fee']*100,2); 

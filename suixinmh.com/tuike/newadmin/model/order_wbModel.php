 ...
 */ 
class order_wbModel extends Model{
  //login form
  ...
  public function orderList($params=array(),$custompage=JIEQI_PAGE_TAG,$emptyonepage = false){
    ...
    $q = jieqi_dbprefix('tuike_orderqd').' o '. 
      ' LEFT JOIN '.jieqi_dbprefix('tuike_qdlistwb').' q ON q.orderqdid=o.id '.
      ' LEFT JOIN '.jieqi_dbprefix('pay_paylog').' p ON p.source=q.qd AND payflag="1" AND p.rettime>=q.statime AND p.rettime<=q.endtime ';
    ...
    while($row=$this->db->getRow($result)){
      $sql='SELECT count(*) FROM '.jieqi_dbprefix('tuike_qdlistwb').' WHERE ( orderqdid="'.$row['id'].'" )';

...
class Controller extends JieqiObject
{
  ...
  final function display($data = array(), $template_name = '', $caching = '', $cacheid = '', $cachetime = '')
  {
    ...
    // 加上异步的参数（给js）
    $filterA=array(
      'limit'=>'10','order'=>'','sort'=>'','page'=>'1','uid'=>'0','payId'=>'0','tkId'=>'0',
      'screen_sort'=>'','screen_t1'=>'','screen_t2'=>'','screen_text'=>'','type'=>'','moon'=>'',
      ); 
    foreach($_REQUEST as $k=>$v){
      if(isset($filterA[$k])){
        if( is_string( $filterA[$k] ) ){

          $filterA[$k]=iconv('GBK', 'UTF-8//IGNORE',$v);
        }else{
    ...
    $this->assign('filter', $filterA );
    $this->assign('ajax_request', isset($_REQUEST['ajax_request'])?1:0);

 ...
 */ 
class manusersdataModel extends Model{
  //login form
   ...
   */
  public function maUserData($params=array(),$custompage=JIEQI_PAGE_TAG,$emptyonepage = false){
    global $cache_redis;
    if( $cache_redis->isExists(JIEQI_REDIS_FIX.'maUserData'.$_SESSION['jieqiUserId']) ){
      
      $ar=unserialize( $cache_redis->get(JIEQI_REDIS_FIX.'maUserData'.$_SESSION['jieqiUserId']) );
      if ($params['pageShow']){
        $this->setVar('totalcount', count($ar));
    ...
    $this->db->criteria->setFields('u.uid,u.uname ');
    ...
    while($row=$this->db->getRow($sqlres)){
      ...
      // 推客数量
      $sql='SELECT count(*) FROM '.jieqi_dbprefix('tuike_users').' WHERE ( groupid=6 AND mauid="'.$row['uid'].'" )';
      $row['tk_num']=$this->db->getField($this->db->query($sql));

      // 上月充值
      $today = getdate();
      $t1 =mktime(0, 0 , 0,$today['mon']-1,1,$today['year']);//获取上月头时间
      $t2 =mktime(23,59,59,$today['mon'] ,0,$today['year']); //获取上月尾时间
      $sql ='SELECT round(sum(p.money)/100,2) as smoney FROM '.jieqi_dbprefix('pay_paylog').' p '.
        'LEFT JOIN '.jieqi_dbprefix('system_qdlist').' q ON p.source=q.qd '.
        'LEFT JOIN '.jieqi_dbprefix('tuike_users').' u ON u.uid=q.uid ';
      $sql.='WHERE( p.rettime >="'.$t1.'" AND p.rettime <="'.$t2.'" AND p.payflag=1 AND u.mauid="'.$row['uid'].'"  )';
      $row['pre_month_pay']=$this->db->getField($this->db->query($sql));
      if(!$row['pre_month_pay'])$row['pre_month_pay']=0;
      ...
      $sql ='SELECT round(sum(p.money)/100,2) as smoney FROM '.jieqi_dbprefix('pay_paylog').' p '.
        'LEFT JOIN '.jieqi_dbprefix('system_qdlist').' q ON p.source=q.qd '.
        'LEFT JOIN '.jieqi_dbprefix('tuike_users').' u ON u.uid=q.uid ';
      $sql.='WHERE( p.rettime >"'.$t2.'" AND p.payflag=1 AND u.mauid="'.$row['uid'].'"  )';
      $row['month_pay']=$this->db->getField($this->db->query($sql));
      if(!$row['month_pay'])$row['month_pay']=0;


      // 近7日充值
      $t2=strtotime($dateS.' -7 day');  
      $sql ='SELECT round(sum(p.money)/100,2) as smoney FROM '.jieqi_dbprefix('pay_paylog').' p '.
        'LEFT JOIN '.jieqi_dbprefix('system_qdlist').' q ON p.source=q.qd '.
        'LEFT JOIN '.jieqi_dbprefix('tuike_users').' u ON u.uid=q.uid ';
      $sql.='WHERE( p.rettime >"'.$t2.'" AND p.payflag=1 AND u.mauid="'.$row['uid'].'"  )';
      $row['seven_day_pay']=$this->db->getField($this->db->query($sql));
      if(!$row['seven_day_pay'])$row['seven_day_pay']=0;
    ...
    $cache_redis->set(JIEQI_REDIS_FIX.'maUserData'.$_SESSION['jieqiUserId'],serialize($ar),$time);
    return $ar;

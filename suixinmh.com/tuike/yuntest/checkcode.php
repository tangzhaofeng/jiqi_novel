...
define('JIEQI_MODULE_NAME', 'manager');
...
define('CHECK_CODE_HEIGHT', 50);  //校验码高度
define('CHECK_CODE_WIDTH', 22);  //校验码宽度（单个字）
define('CHECK_CODE_SPACEX', 10);  //校验码左边空
define('CHECK_CODE_SPACEY', 10);  //校验码顶部空
define('CHECK_CODE_SPACEM', 1);  //校验码字符间距3
header("Content-type: image/png");

$digits = array(95, 5, 118, 117, 45, 121, 123, 69, 127, 125);
$digits = array(95, 125, 118, 117, 125, 121, 123, 125, 127, 125);
$digits = array(1);
...
if($jieqiConfigs['system']['usegd'] && function_exists('gd_info')){
	...
	$back = imagecolorallocate($im, mt_rand(222,255),mt_rand(222,255),mt_rand(222,255));//
	$gray = $white;
	...
	for ($i=0;$i<strlen($checkcode);$i++)
	{
		...
		$p = array_rand($digits);
		$an = $array[$p]*mt_rand(10,13);//角度
		$an = 1;//角度
		$size = mt_rand(27,28);
		$white = imagecolorallocate($im, mt_rand(0,100),mt_rand(0,100),mt_rand(0,100));
		imagettftext($im, $size, $an, $i*$size, $size+CHECK_CODE_SPACEY, $white, $font, $tmp);
	}

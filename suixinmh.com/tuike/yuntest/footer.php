...
if (!empty($_REQUEST['ajax_request']) && !empty($_REQUEST['ajax_gets'])) {
    ...
    if (isset($_REQUEST['CALLBACK']) && !empty($_REQUEST['CALLBACK'])) {
        ...
        die(json_encode(array('status'=>'OK','filter'=>$jieqiTpl->_tpl_vars['filter'])).'aj||ax'.$out_var);
    }
}
...
if (defined('JIEQI_DEBUG_MODE') && JIEQI_DEBUG_MODE > 0) {
    ...
    echo '<div id="cloosDebug" class="divbox"><p style="background-color:#ccc;" onclick="cloosDebug.style.display=\'none\';"><a>X</a></p>' . $debuginfo . '</div>';
    echo '
<style>
.divbox{
    position: fixed;

...
if(!defined('JIEQI_ROOT_PATH')) @define('JIEQI_ROOT_PATH', str_replace(array('\\\\', '\\'), '/', dirname( dirname(__FILE__) )));
if(!defined('JIEQI_ROOT_PATH_APP'))define('JIEQI_ROOT_PATH_APP', JIEQI_ROOT_PATH.'/'.JIEQI_MODULE_NAME);

include_once(JIEQI_ROOT_PATH_APP.'/configs/define.php');
...
$jieqiModules = array();
include_once(JIEQI_ROOT_PATH_APP.'/configs/modules.php');
...
if (!empty($_COOKIE[session_name()]) || defined('JIEQI_NEED_SESSION')) {
    ...
    elseif (JIEQI_SESSION_STORAGE == 'redis') {//echo JIEQI_SESSION_SAVEPATH;exit;
    ...
    @session_start();
    if (!empty($_COOKIE[session_name()]) && !empty($_COOKIE['jieqiUserInfoTk']) && count($_SESSION) == 0) {
...
if (empty($_SESSION['jieqiUserId'])) {
    ...
    if (!empty($_REQUEST['system_username']) && !empty($_REQUEST['system_userpassword'])) {
    ...
    } elseif (!empty($_COOKIE['jieqiUserInfoTk'])) {
        $jieqi_user_info = jieqi_strtosary($_COOKIE['jieqiUserInfoTk']);
        ...
        } else @setcookie('jieqiUserInfoTk', '', time() - 100, '/', JIEQI_COOKIE_DOMAIN, 0);
    }
}
...
if (JIEQI_MODULE_NAME != 'news') {
    include_once(JIEQI_ROOT_PATH_APP . '/core/app.php');
}
...
function jieqi_customerror($errno, $errstr, $errfile, $errline)
{
    ...
    switch ($errno) {
        case E_USER_ERROR:
            ...
            echo "�����У�{$errline} ���ļ���{$errfile} ֮��<br/>";
            echo " PHP�汾�� " . PHP_VERSION . " (" . PHP_OS . ")<br/>";
            break;
        case E_USER_WARNING:
...
function jieqi_setusersession($user)
{
    ...
    $_SESSION['jieqiUserHead'] = $user->getVar('head', 'n');
    $_SESSION['jieqiUserBalance'] = $user->getVar('balance', 'n');
    $_SESSION['jieqiUserBalancey'] = $user->getVar('balancey', 'n');
    $_SESSION['jieqiUserBalancer'] = $user->getVar('balancer', 'n');
    $_SESSION['jieqiUserMauid'] = $user->getVar('mauid', 'n');
    $_SESSION['is_tuike'] = $user->getVar('is_tuike', 'n');
...
class JieqiObject
{
    ...
    function submitcheck($var = 'formhash')
    {
        ...
        if (!$this->checkisadmin() && !empty($_REQUEST[$var]) && $ip != '113.140.9.50') {
            ...
            if ($ip_arrays = @file($deniedip)) {
                ...
                if (in_array($ip, $a_tmp)) $this->printfail('���ѱ�ϵͳ���Ʋ�����');
            }
            ...
            if ($_SESSION['submitcount'] <= $count) {
                ...
                if (!is_object($db) && class_exists('Application')) {
                    ...
                    $data['toname'] = '';
                    $data['reason'] = '�������ݻ����';
                    $data['chginfo'] = $ip . "��һ������������������������" . $_REQUEST['controller'] . " ������" . $_REQUEST['method'] . "������������ " . ($_SESSION['submitcount']) . " �α�������!�ж�Ϊ�����û������û��Ѿ���" . ($_SESSION['jieqiUserId'] ? '��Ϊ�ο�' : '��ֹ�ύ����') . "��";
                    $data['chglog'] = '';
                    $data['isdel'] = '0';
                    $data['userlog'] = '';
                    $db->add($data);
                ...
                $this->printfail('ϵͳ���⵽�Ƿ�������Դ������IP:' . $ip . '�����Ѿ���¼���������뿪��');
            }
        }             
    ...
    function getAuth()
    {
        global $jieqiGroups;
        $_USER = array();
        if ( isset($_SESSION['jieqiUserId']) ) {
            $_USER['uid'] = intval($_SESSION['jieqiUserId']);
            $_USER['useruname'] = addslashes($_SESSION['jieqiUserUname']);
            $_USER['useremail'] = addslashes($_SESSION['jieqiUserEmail']);
            $_USER['userhead'] = addslashes($_SESSION['jieqiUserHead']);
            $_USER['userbalance'] = $_SESSION['jieqiUserBalance'];
            $_USER['userbalancey'] = $_SESSION['jieqiUserBalancey'];
            $_USER['userbalancer'] = $_SESSION['jieqiUserBalancer'];
            $_USER['mauid'] = $_SESSION['jieqiUserMauid'];
            $_USER['is_tuike'] = $_SESSION['is_tuike'];
...
class StopAttack extends JieqiObject
{
    ...
    public function __construct()
    {
        ...
        if ( !isset($_SESSION['jieqiAdminLogin']) ) {
    ...
    public function checkparams($StrFiltKey, $StrFiltValue, $ArrFiltReq)
    {
        ...
        if (preg_match("/" . $ArrFiltReq . "/is", $StrFiltValue) == 1) {
            $this->swritefile(JIEQI_ROOT_PATH . '/files/stopattack_log.html', "<br><hr>����IP: " . $_SERVER["REMOTE_ADDR"] . "<br>����ʱ��: " . strftime("%Y-%m-%d %H:%M:%S") . "<br>����ҳ��:" . $_SERVER["PHP_SELF"] . "<br>�ύ��ʽ: " . $_SERVER["REQUEST_METHOD"] . "<br>�ύ����: " . $StrFiltKey . "<br>�ύ����: " . $StrFiltValue . "\r\n", "a+");
            $this->printfail('�����ύ���в��Ϸ�����,лл����!');
        }
        if (preg_match("/" . $ArrFiltReq . "/is", $StrFiltValue) == 1) {
            $this->swritefile(JIEQI_ROOT_PATH . '/files/stopattack_log.html', "<br><hr>����IP: " . $_SERVER["REMOTE_ADDR"] . "<br>����ʱ��: " . strftime("%Y-%m-%d %H:%M:%S") . "<br>����ҳ��:" . $_SERVER["PHP_SELF"] . "<br>�ύ��ʽ: " . $_SERVER["REQUEST_METHOD"] . "<br>�ύ����: " . $StrFiltKey . "<br>�ύ����: " . $StrFiltValue . "\r\n", "a+");
            $this->printfail('�����ύ���в��Ϸ�����,лл����!');

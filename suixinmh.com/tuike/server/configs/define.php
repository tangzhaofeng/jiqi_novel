...
@define('JIEQI_PAGE_TAG','<div> <span>{$totalcount} 条记录 {$page}/{$totalpage} 页</span> [firstpage] <a class="end" href="{$firstpage}">1</a>[/firstpage][prepage] <a class="next" href="{$prepage}">&lt;&lt;</a> [/prepage] [pages] [pnumchar] <span class="current">{$page}</span> [/pnumchar] [pnumurl] <a class="num" href="{$pnumurl}">{$pagenum}</a> [/pnumurl] {$pages} [/pages] [nextpage] <a class="next" href="{$nextpage}">&gt;&gt;</a> [/nextpage] [lastpage] <a class="end" href="{$lastpage}">{$totalpage}</a> [/lastpage] </div>');

...
/**
 * 检查本次点击时候有效的点击
 * <br>session和cookie生命周期内都不存在点击记录的id（书号）算一次点击，然后session和cookie记录这次点击量
 * @param      int         $id 点击对象ID
...
function jieqi_visit_valid($id, $vname, $save=true){
	...
	if($save){
		...
		if(!in_array($id,$arycookie)){
			...
			setcookie('jieqiVisitId', jieqi_sarytostr($tmpary, '=', ','), JIEQI_NOW_TIME+60, '/',  JIEQI_COOKIE_DOMAIN, 0);
		}
	}
	return true;
}
 ...
 * 返回点击统计数组，支持点击缓存
 * 
 * @param      int         $id 点击文章的statid
 * @param      string      $vname 点击标志变量名
 * @param      int         $lastvisit 上一次更新访问统计时间（需要分周月总统计时候用到）
 * @access     public
 * @return     mixed       如果缓存本点击，返回false，否则返回数组?嗨芶rray('1'=>array('visitnum'=>2, 'lastvisit'=>'12345678'), '5'=>array('visitnum'=>1, 'lastvisit'=>'12345678')),这里的键值表示记录ID，visitnum表示被访问了几次，lastvisit是最后更新访问统计的时间
 */
function jieqi_visit_ids($id, $vname, $lastvisit=-1){
	...
	if (JIEQI_ENABLE_CACHE){
		...
		//启用缓存，百分之一机会更新缓存，通过随机数为1时返回缓存的ids
		if(rand(1, 10) == 1) {

...
function jieqi_registerdo($gourl, $jump=true){
	global $jieqiLang;
	if($jump) jieqi_jumppage($gourl, $jieqiLang['system']['registered_title'], sprintf($jieqiLang['system']['register_success'], jieqi_htmlstr($_REQUEST['username'])));
 ...
 * @return     void
 */
 function jieqi_passeditdo($gourl, $jump=true){
	global $jieqiLang;
	if($jump) jieqi_jumppage($gourl, LANG_DO_SUCCESS, $jieqiLang['system']['pass_edit_success']);
	else header('Location: '.$gourl);
}
?>
...
//修改密码后的操作
function jieqi_passeditdo($gourl){
	global $jieqiLang;
	if(strpos($gourl, 'http') === false){
		if($_SERVER['HTTP_HOST'] != '') $gourl='http://'.$_SERVER['HTTP_HOST'].$gourl;
		else $gourl=JIEQI_URL.$gourl;
	...
	//通过接口同步修改密码
	$ucsynlogout = uc_user_edit($_SESSION['jieqiUserUname'], $_REQUEST['oldpass'], $_REQUEST['newpass'], $_SESSION['jieqiUserEmail']);
	jieqi_jumppage($gourl, LANG_DO_SUCCESS, $jieqiLang['system']['pass_edit_success']);

...
function swfupload(id,inputid,title,isadmin,more,isthumb,file_limit,file_types,moduleid,auth,yesdo,nodo,url){ 
...
function up_images(iframeWin, topWin,id,inputid){ 
		...
		if(num){
			iframeWin.$('#myuploadform  div ').each(function(){
					...
					data += '<div id="uplist_'+aid+'">'+aidinput+'<input type="text" size="50" class="input-text" name="'+inputid+'[]" value="'+src+'"  />  <input type="text" class="input-text" name="'+inputid+'_name[]" value="'+name+'" size="30" /> &nbsp;<a href="javascript:remove_this(\'uplist_'+aid+'\');">ÒÆ³ý</a> </div>';
			});			

<?php 
/** 
 * 系统管理->系统信息 * @copyright   Copyright(c) 2014 
 * @author      gaoli* @version     1.0 
 */ 
jieqi_loadlang('sysinfo', 'system');
class sysinfoModel extends Model{
        public function getData(){
			...
			if(floatval($this->jieqi_zendoptimizerver()) < 3) $state='error';
			else $state='ok';
			$sysinfos[]=array('name'=>$jieqiLang['system']['sinfo_zend_optimizer'], 'value'=>$this->jieqi_zendoptimizerver(), 'note'=>$jieqiLang['system']['snote_zend_optimizer'], 'state'=>$state);
			...
			$sysinfos[]=array('name'=>$jieqiLang['system']['sinfo_register_globals'], 'value'=>$this->jieqi_issupport(get_cfg_var("register_globals")), 'note'=>$jieqiLang['system']['snote_register_globals'], 'state'=>$state);

			//魔术引用（不推荐）get_cfg_var('magic_quotes_gpc');
			$sysinfos[]=array('name'=>$jieqiLang['system']['sinfo_magic_quotes'], 'value'=>$this->jieqi_issupport(get_cfg_var('magic_quotes_gpc')), 'note'=>$jieqiLang['system']['snote_magic_quotes'], 'state'=>'ok');
			...
			$sysinfos[]=array('name'=>$jieqiLang['system']['sinfo_display_errors'], 'value'=>$this->jieqi_issupport(get_cfg_var("display_errors")), 'note'=>$jieqiLang['system']['snote_display_errors'], 'state'=>$state);

			//SMTP支持 get_magic_quotes_gpc("smtp")
			$sysinfos[]=array('name'=>$jieqiLang['system']['sinfo_smtp_support'], 'value'=>$this->jieqi_issupport(get_magic_quotes_gpc("smtp")), 'note'=>$jieqiLang['system']['snote_smtp_support'], 'state'=>'ok');

			//PHP安全模式 get_cfg_var("safe_mode")
			if(get_cfg_var("safe_mode")) $state='notice';
			else $state='ok';
			$sysinfos[]=array('name'=>$jieqiLang['system']['sinfo_safe_mode'], 'value'=>$this->jieqi_issupport(get_cfg_var("safe_mode")), 'note'=>$jieqiLang['system']['snote_safe_mode'], 'state'=>$state);

			//XML语法解析库 function_exists('xml_parser_set_option');
			if(function_exists('xml_parser_set_option')) $state='ok';
			else $state='error';
			$sysinfos[]=array('name'=>$jieqiLang['system']['sinfo_xml_parser'], 'value'=>$this->jieqi_issupport(function_exists('xml_parser_set_option')), 'note'=>$jieqiLang['system']['snote_xml_parser'], 'state'=>$state);

			//XML 解析函数库 get_magic_quotes_gpc("XML Support")
			$sysinfos[]=array('name'=>$jieqiLang['system']['sinfo_xml_support'], 'value'=>$this->jieqi_issupport(get_magic_quotes_gpc("XML Support")), 'note'=>$jieqiLang['system']['snote_xml_support'], 'state'=>'ok');

			//FTP 文件传输函数库 get_magic_quotes_gpc("FTP support")
			$sysinfos[]=array('name'=>$jieqiLang['system']['sinfo_ftp_support'], 'value'=>$this->jieqi_issupport(get_magic_quotes_gpc("FTP support")), 'note'=>$jieqiLang['system']['snote_ftp_support'], 'state'=>'ok');

			//允许使用URL打开文件(采集需要，不采集不用) get_cfg_var("allow_url_fopen")
			if(get_cfg_var("allow_url_fopen")) $state='ok';
			else $state='warning';
			$sysinfos[]=array('name'=>$jieqiLang['system']['sinfo_url_fopen'], 'value'=>$this->jieqi_issupport(get_cfg_var("allow_url_fopen")), 'note'=>$jieqiLang['system']['snote_url_fopen'], 'state'=>$state);

			//动态链接库 get_cfg_var("enable_dl")
			$sysinfos[]=array('name'=>$jieqiLang['system']['sinfo_enable_dl'], 'value'=>$this->jieqi_issupport(get_cfg_var("enable_dl")), 'note'=>$jieqiLang['system']['snote_enable_dl'], 'state'=>'ok');

			//IMAP 电子邮件系统函数库 function_exists("imap_close")
			$sysinfos[]=array('name'=>$jieqiLang['system']['sinfo_imap_support'], 'value'=>$this->jieqi_issupport(function_exists("imap_close")), 'note'=>$jieqiLang['system']['snote_imap_support'], 'state'=>'ok');

			//历法运算函数库 function_exists("JDToGregorian")
			$sysinfos[]=array('name'=>$jieqiLang['system']['sinfo_calendar_support'], 'value'=>$this->jieqi_issupport(function_exists("JDToGregorian")), 'note'=>$jieqiLang['system']['snote_calendar_support'], 'state'=>'ok');

			//压缩文件函数库(Zlib) function_exists("gzclose")
			if(function_exists("gzclose")) $state='ok';
			else $state='warning';
			$sysinfos[]=array('name'=>$jieqiLang['system']['sinfo_zlib_support'], 'value'=>$this->jieqi_issupport(function_exists("gzclose")), 'note'=>$jieqiLang['system']['snote_zlib_support'], 'state'=>$state);

			//Session支持 function_exists("session_start")
			if(function_exists("session_start")) $state='ok';
			else $state='error';
			$sysinfos[]=array('name'=>$jieqiLang['system']['sinfo_session_support'], 'value'=>$this->jieqi_issupport(function_exists("session_start")), 'note'=>$jieqiLang['system']['snote_session_support'], 'state'=>$state);

			//Socket支持 function_exists("fsockopen")
			if(function_exists("fsockopen")) $state='ok';
			else $state='warning';
			$sysinfos[]=array('name'=>$jieqiLang['system']['sinfo_socket_support'], 'value'=>$this->jieqi_issupport(function_exists("fsockopen")), 'note'=>$jieqiLang['system']['snote_socket_support'], 'state'=>$state);

			//正则表达式函数库 function_exists("preg_match")
			if(function_exists("preg_match")) $state='ok';
			else $state='error';
			$sysinfos[]=array('name'=>$jieqiLang['system']['sinfo_preg_support'], 'value'=>$this->jieqi_issupport(function_exists("preg_match")), 'note'=>$jieqiLang['system']['snote_preg_support'], 'state'=>$state);


			//图像函数库 function_exists("imageline")
			if(function_exists("imageline")) $state='ok';
			else $state='notice';
			$sysinfos[]=array('name'=>$jieqiLang['system']['sinfo_gd_support'], 'value'=>$this->jieqi_issupport(function_exists("gd_info")), 'note'=>$jieqiLang['system']['snote_gd_support'], 'state'=>$state);
			...
			if(function_exists("gd_info")){
				...
				$sysinfos[]=array('name'=>$jieqiLang['system']['sinfo_freetype_support'], 'value'=>$this->jieqi_issupport($tmpary['FreeType Support']), 'note'=>$jieqiLang['system']['snote_freetype_support'], 'state'=>$state);
	
				if($tmpary['GIF Read Support']) $state='ok';
				else $state='notice';
				$sysinfos[]=array('name'=>$jieqiLang['system']['sinfo_gif_read'], 'value'=>$this->jieqi_issupport($tmpary['GIF Read Support']), 'note'=>$jieqiLang['system']['snote_gif_read'], 'state'=>$state);
	
				if($tmpary['GIF Create Support']) $state='ok';
				else $state='notice';
				$sysinfos[]=array('name'=>$jieqiLang['system']['sinfo_gif_create'], 'value'=>$this->jieqi_issupport($tmpary['GIF Create Support']), 'note'=>$jieqiLang['system']['snote_gif_create'], 'state'=>$state);
	
				if($tmpary['JPG Support']) $state='ok';
				else $state='notice';
				$sysinfos[]=array('name'=>$jieqiLang['system']['sinfo_jpg_support'], 'value'=>$this->jieqi_issupport($tmpary['JPG Support']), 'note'=>$jieqiLang['system']['snote_jpg_support'], 'state'=>$state);
	
				if($tmpary['PNG Support']) $state='ok';
				else $state='notice';
				$sysinfos[]=array('name'=>$jieqiLang['system']['sinfo_png_support'], 'value'=>$this->jieqi_issupport($tmpary['PNG Support']), 'note'=>$jieqiLang['system']['snote_png_support'], 'state'=>$state);
	
				if($tmpary['WBMP Support']) $state='ok';
				else $state='notice';
				$sysinfos[]=array('name'=>$jieqiLang['system']['sinfo_wbmp_support'], 'value'=>$this->jieqi_issupport($tmpary['WBMP Support']), 'note'=>$jieqiLang['system']['snote_wbmp_support'], 'state'=>$state);
			}

			//Iconv编码转换 function_exists("iconv")
			$sysinfos[]=array('name'=>$jieqiLang['system']['sinfo_iconv_support'], 'value'=>$this->jieqi_issupport(function_exists("iconv")), 'note'=>$jieqiLang['system']['snote_iconv_support'], 'state'=>'ok');

			//MySQL 数据库 function_exists("mysql_close")
			if(function_exists("mysql_close")) $state='ok';
			else $state='error';
			$sysinfos[]=array('name'=>$jieqiLang['system']['sinfo_mysql_support'], 'value'=>$this->jieqi_issupport(function_exists("mysql_close")), 'note'=>$jieqiLang['system']['snote_mysql_support'], 'state'=>$state);
			...
			//getmygid() 不推荐
			 //print_r($this->db->lists());exit;
			 return $sysinfos;
        }

	//zend optimizer 版本
	public function jieqi_zendoptimizerver(){
		ob_start();
		phpinfo();
	...
	//是否支持
	public function jieqi_issupport($var){
		global $jieqiLang;

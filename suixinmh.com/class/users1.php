...
$system_users_fields['idnumber']=array('name'=>'IDNumber', 'type'=>JIEQI_TYPE_TXTBOX, 'value'=>'', 'caption'=>'身份证', 'required'=>false, 'maxlength'=>15);
...
// 		print_r($db);
	...
	function getByname($name,$flag=1){
		if (!empty($name)) {
			...
			if($flag==3) $sql = "SELECT * FROM ".jieqi_dbprefix($this->dbname, $this->fullname)." WHERE ".$this->tableFields['uname']['name']."='".$name."' UNION ALL SELECT * FROM ".jieqi_dbprefix($this->dbname, $this->fullname)." WHERE ".$this->tableFields['name']['name']."='".$name."'";// ORDER BY name DESC";
...
/*		if($useregold >= $emoney){
	...
	{
		$tmpuser=$this->get($uid);
		if($addscore>0) $addscore = $this->accelerationScore($tmpuser,$addscore);//积分加速
		if(is_object($tmpuser)){
			//增加虚拟币
			if($usesliver==1) $tmpuser->setVar('esilver', bcsub($tmpuser->getVar('esilver'),$emoney,2));
			else $tmpuser->setVar('egold', bcsub($tmpuser->getVar('egold'),$emoney,2));
			//修改vip成长值
			//$updatevip=intval($updatevip); && $tmpuser->getVar('isvip')<$updatevip
			if($updatevip>0) $tmpuser->setVar('isvip',$tmpuser->getVar('isvip')+ $updatevip);
			//增加积分

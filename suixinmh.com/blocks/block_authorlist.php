...
class BlockSystemAuthorlist extends JieqiBlock
{
	var $module='system';
	...
	function BlockSystemAuthorlist(&$vars){
	...
	function setContent($isreturn=false){
		...
		$sql ="select uid,name,uname,f.num from `jieqi_system_users`  u,".
		"(SELECT yourid,count( * ) AS 'num' FROM `jieqi_system_friends` GROUP BY yourid)  f  where u.uid = f.yourid and (u.groupid=6 or u.groupid = 7)  order by num desc limit 0,".$this->exevars['listnum'];
		$userrows = $this->db->selectsql($sql);
	/*	$users_handler = $this->getUserObject();
		$criteria=new CriteriaCompo();
		if(!empty($this->exevars['groupid'])){
			$groupary=explode('|', $this->exevars['groupid']);
			foreach($groupary as $v){
				if(is_numeric($v) && ($v == 6 || $v == 7 )) $criteria->add(new Criteria('groupid', intval($v), '='), 'OR');
			}
		}
		...
		while($v = $users_handler->getObject()){
			...
			$this->db->setCriteria(new Criteria ( 'myid',$userrows[$k]['uid'], '=' ));
			
			$userrows[$k]['guanzhuliang']=$this->db->getCount($this->db->criteria);
			//徽章

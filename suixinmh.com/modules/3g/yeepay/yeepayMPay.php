...
//	正式环境请求基础地址
	 ...
	 * @return string
	 */
	public function webPay($order_id,$transtime,$amount,$product_catalog,$identity_id,$identity_type,$user_ip,$user_ua,$callbackurl='',$fcallbackurl='',$product_name='',$terminaltype,$terminalid,$version){
		$query = array(
			'orderid'	=>	$order_id,
			'transtime'	=>	$transtime,
			'amount'	=>	$amount,
...
//			'owner' => $owner,
	...
	*/
	 ...
	 * @return array
	 */
	public function callback($data,$encryptkey){//, &$dat['reason']
//		$dat['reason'] .= '111';echo 'callback ';
		return $this->parseReturn($data, $encryptkey, $dat['reason']);//
	}
...
//		for api : query/order
	...
	protected function parseReturn($data,$encryptkey, &$testData){//
//		$testData .= 'thisParseReturn+';echo 'parsereturn ';
		$AESKey = $this->getYeepayAESKey($encryptkey, $testData);//
...
//		unset($return['sign']);
	 ...
	 */
	protected function getYeepayAESKey($encryptkey, &$testData){
		$testData .= 'getYeep+';
		$this->RSA->loadKey($this->merchantPrivateKey);
		$yeepayAESKey = $this->RSA->decrypt(base64_decode($encryptkey), $testData);
		return $yeepayAESKey;
	}
	/**
	 * 通过AES加密请求数据

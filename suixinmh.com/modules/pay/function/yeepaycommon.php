...
function getReqHmacString($orderId,$amount,$cur,$productId,$productCat,$productDesc,$sMctProperties,$frpId,$needResponse,$params)
{
//	$args = func_get_args();
//	print_r($args);
  extract($params);
/*  global $nodeAuthorizationURL;
  ...
  global $keyValue;
*/      
//echo 	$nodeAuthorizationURL.' | '.$messageType.' | '.$addressFlag.' | '.$merchantId;echo $nodeAuthorizationURL;exit('jj');
  #进行加密串处理，一定按照下列顺序进行
  #取得加密前的字符串
  #加入消息类型
  $sbOld="";
...
function getCallbackHmacString($sCmd,$sErrorCode,$sTrxId,$orderId,$amount,$cur,$productId,$userId,$MP,$bType,$keyValue,$merchantId){
//    echo "<hr>";
//	$args = func_get_args();
//	print_r($args);
...
function CheckHmac($sCmd,$sErrorCode,$sTrxId,$orderId,$amount,$cur,$productId,$userId,$MP,$bType,$svrHmac,$keyValue,$merchantId){
//    $args = func_get_args();
//	print_r($args);
//echo "<br>";
//echo $svrHmac."<br>";
//echo getCallbackHmacString($sCmd,$sErrorCode,$sTrxId,$orderId,$amount,$cur,$productId,$userId,$MP,$bType);
	if($svrHmac==getCallbackHmacString($sCmd,$sErrorCode,$sTrxId,$orderId,$amount,$cur,$productId,$userId,$MP,$bType,$keyValue,$merchantId))
		return true;
	else
		return false;
}

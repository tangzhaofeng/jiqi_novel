<?php
include '../../configs/pay/yeepay.php';
/*
...
function getReqHmacString($p2_Order,$p3_Amt,$p4_Cur,$p5_Pid,$p6_Pcat,$p7_Pdesc,$p8_Url,$pa_MP,$pd_FrpId,$pr_NeedResponse)
{
  global $p0_Cmd;
  include '../../configs/pay/yeepay.php';
		$p0_Cmd = "Buy";exit($p0_Cmd.'hhhhh');
	//进行签名处理，一定按照文档中标明的签名顺序进行
  $sbOld = "";
  //加入业务类型
  $sbOld = $sbOld.$p0_Cmd;
  //加入商户编号
  $sbOld = $sbOld.$jieqiPayset['yeepay']['payid'];
  //加入商户订单号
  ...
  $sbOld = $sbOld.$p8_Url;
  //加入送货地址标识
  $sbOld = $sbOld.$jieqiPayset['yeepay']['addressFlag'];
  //加入商户扩展信息
  $sbOld = $sbOld.$pa_MP;
  //加入支付通道编码
  ...
  logstr($p2_Order,$sbOld,HmacMd5($sbOld,$jieqiPayset['yeepay']['paykey']));
  return HmacMd5($sbOld,$jieqiPayset['yeepay']['paykey']);
...
function getCallbackHmacString($r0_Cmd,$r1_Code,$r2_TrxId,$r3_Amt,$r4_Cur,$r5_Pid,$r6_Order,$r7_Uid,$r8_MP,$r9_BType){
	...
	logstr($r6_Order,$sbOld,HmacMd5($sbOld,$jieqiPayset['yeepay']['paykey']));
	return HmacMd5($sbOld,$jieqiPayset['yeepay']['paykey']);

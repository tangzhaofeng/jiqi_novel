...
// 		}else{
			...
			$this->db->criteria->setFields('c.*, a.articleid, a.lastupdate, a.articlename, a.authorid, a.author, a.sortid, a.lastchapterid, a.lastchapter, a.fullflag,a.imgflag');
...
/*		$this->instantPackage($aid);
	...
	*/
	public function getAllVip($aid)
	{
		...
		$this->db->criteria->add(new Criteria('c.articleid', $aid, '='));
		$this->db->queryObjects($this->db->criteria);
	...
	function makeHtml($nowid, $show=false, $filter=false, $obj = NULL){
		...
		if(!isset($jieqiLang)){
		 ...
		 //非本频道书跳转到相应频道
		 if(!in_array(JIEQI_MODULE_NAME,array('wap','3gwap')) && $show){//echo $this->getModule($data['article']['sortid']);exit;

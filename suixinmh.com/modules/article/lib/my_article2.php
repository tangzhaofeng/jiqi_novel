...
// 		$this->jieqiConfigs['channel'] = $this->getConfig('article','channel');
	...
	private function calculatorPrice($article,&$newChapter,$chaptersize,$saleprice){
		//isvip,saleprice
			...
			if(!empty($article) && !empty($newChapter)){// && !empty($saleprice)
				if(trim($saleprice) != ''  && !empty($saleprice) && $this->checkpower($this->jieqiPower['article']['customprice'], $this->getUsersStatus (), $this->getUsersGroup (), true)){
					...
					$this->jieqiConfigs['article']['wordsperegold']=ceil($this->jieqiConfigs['article']['wordsperegold']) * 2;//2倍整数
					if($this->jieqiConfigs['article']['priceround']==1){
						$newChapter ['saleprice']=floor($chaptersize / $this->jieqiConfigs['article']['wordsperegold']);//向下舍入，取整。
					}elseif($this->jieqiConfigs['article']['priceround']==2){
						$newChapter ['saleprice']=ceil($chaptersize / $this->jieqiConfigs['article']['wordsperegold']);//向上舍入，取整。
					}else{
						$newChapter ['saleprice']=round($chaptersize / $this->jieqiConfigs['article']['wordsperegold']);//四舍五入
					}
				}
			}
	}
	...
	private function chapterInsideHandle($params) {
		...
		if(((int)date('H')<8 && (int)date('H')>=0) || (int)date('H')>=23) $postnum = 9;
...
/*		$this->instantPackage($aid);
	...
	function makeHtml($nowid, $show=false, $filter=false, $obj = NULL){
		...
		if(!isset($jieqiLang)){
		 ...
		 //非本频道书跳转到相应频道
		 if(!in_array(JIEQI_MODULE_NAME,array('wap')) && $show){
		...
		if(!$this->chapters[$nowid-1]['display'] && !$data['article']['display'] ){
			if(!$data['chapter']['isvip'] || (defined('RETURN_READER_CONTENT') && RETURN_READER_CONTENT) || ($data['chapter']['isvip'] && !$data['chapter']['saleprice'] && defined('RETURN_READER_DATA') && RETURN_READER_DATA)){//不是VIP才生成内容
...
// 		$data['chapter']['index_page'] = ($this->geturl('article', 'index', 'aid='.$this->id));//basename
		...
		if($data['chapter']['isvip']>0){//如果是VIP章节，则显示另一套模板
			...
			$data['chapter']['readvip'] = $this->geturl('article', 'reader', 'method=readvip', 'aid='.$this->id,'cid='.$chapterid,'SYS=method=readvip');
		}
		if($show){
	...
	function article_vars($article){
		...
		if(is_object($article)){
			foreach($article->getVars() as $k=>$v){
			 ...
			 $ret['intro']=(htmlspecialchars_array($this->substr($article->getVar('intro', 'n'),0, 600)));//jieqi_htmlstr
			 $ret['notice']=(htmlspecialchars_array($this->substr($article->getVar('notice', 'n'),0, 600)));
			...
			return $ret;
			 ...
			 if($article['lastchapter']==''){
				  ...
				  $article['intro']=(htmlspecialchars_array(jieqi_substr($article['intro'],0, 350)));//jieqi_htmlstr
				  $article['notice']= empty($article['notice'])? '':(htmlspecialchars_array(jieqi_substr($article['notice'],0, 600)));
	...
	function addArticleStat($articleid,$authorid, $mid, $stat = 1, $ext = array()){
		 ...
		 if($statObj = $this->db->get($this->db->criteria)){//如果存在则修改
			  ...
			  $total = $statObj->getVar('total', 'n')+$stat;
			  $totalnum = $statObj->getVar('totalnum', 'n')+1;
			  if($nowdate==$lastdate){
			       $month = $statObj->getVar('month', 'n')+$stat;
				   $week = $statObj->getVar('week', 'n')+$stat;
				   $day = $statObj->getVar('day', 'n')+$stat;
	...
	function getStatArray(){
	     return array(
			  'visit'=>array('name'=>'点击','format'=>'访问了作品《<a href="{$url_articleinfo}" target="_blank">{$articlename}</a>》。'),
			  'vote'=>array('name'=>'推荐','format'=>'给作品《<a href="{$url_articleinfo}" target="_blank">{$articlename}</a>》投了{$stat}张推荐票。'),
		      'goodnum'=>array('name'=>'收藏','format'=>'把作品《<a href="{$url_articleinfo}" target="_blank">{$articlename}</a>》放入了书架。'),
		      'vipvote'=>array('name'=>'月票','format'=>'给作品《<a href="{$url_articleinfo}" target="_blank">{$articlename}</a>》投了{$stat}张月票。'),
			  'sale'=>array('name'=>'销售','format'=>'订阅了作品《<a href="{$url_articleinfo}" target="_blank">{$articlename}</a>》的章节：<a href="{$url_chapterurl}" target="_blank">{$chaptername}</a>。'),
			  'reward'=>array('name'=>'打赏','format'=>'大方的打赏了作品《<a href="{$url_articleinfo}" target="_blank">{$articlename}</a>》{$stat}'.JIEQI_EGOLD_NAME.'。')
		 );
	}
	function getStatStr(){
	     $type = array();

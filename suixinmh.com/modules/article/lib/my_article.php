...
// 		}
	...
	private function chapterInsideHandle($params) {
		...
		if(((int)date('H')<8 && (int)date('H')>=0) || (int)date('H')>=23) $postnum = 100;
		elseif((int)date('H')>=18 && (int)date('H')<=23) $postnum = 200;
		elseif((int)date('H')>=9 && (int)date('H')<=18) $postnum = 300;
		$ip = $this->getIp();
		if($this->db->getCount($this->db->criteria)>=$postnum && $auth['groupid']!=2){    
			$ip = $ip ? $ip : 'allip';
...
// 			}
			...
			if(isset($data['oldaid'])) $newArticle ['oldaid'] = $data ['oldaid'] ? $data ['oldaid'] : 0;
...
// 			$this->jieqiConfigs ['article'] ['pagenum'] = 1;
		...
		$data ['articlerows'] = $this->db->lists ( 100, $page,JIEQI_PAGE_TAG);
		// 处理页面跳转
...
//                $chapters_array = array();
				...
				$this->write_cache_php($article,$chapters_array);
		 		// 开始生成
		 		$this->isload = true;
		 		// 生成opf
...
/*		$this->instantPackage($aid);
	...
	*/
	public function checkChapterIsBuy($cid){
	    ...
	    $auth = $this->getAuth();
		if ($auth['groupid'] == 5 && $auth['overtime'] >= time()) {
            return true;
        }
	...
	*/
	public function batchlist($aid, $chapter)
	{
		...
		while($v = $this->db->getObject())
		{
			...
			$useshuquan = 1;  //in_array($arr[$i]['chapterid'], $tmpchapters) ? 1 : 0;
	...
	public function getChaptersLimit($aid, $limit = 10){
        ...
        $cache_data = $this->redis->get("vip_chapter_list_".$aid);
        if ($cache_data) {
            $cids = unserialize($cache_data);
            ...
            $cache_data = serialize($cids);
            $this->redis->set('vip_chapter_list_'.$aid,$cache_data);
            $this->chaptersLimit[$aid] = $cids;
            return $cids;
        }
	}
	...
	*
	*/
	public function getAllVip($aid, $cids = array(),$from_cid=0)
	{
		$auth = $this->getAuth();
		$saleprice = 0;
		$buyArr = array();
        ...
        if ($from_cid) {
            $this->db->init("chapter","chapterid","article");
            $from_chapter = $this->db->get($from_cid);
            $from_chapterorder = $from_chapter['chapterorder'];
        }
        else {
            $from_chapterorder = 0;
        }
		...
		$auth = $this->getAuth();
		$useregold=$auth['egolds'];//书币余额
		$useresilver=$auth['esilvers'];//书券余额

        $useregold_tmp = $useregold;
        $useresilver_tmp = $useresilver;
		...
		$this->db->criteria->add(new Criteria('articleid', $aid, '='));
		$this->db->criteria->add(new Criteria('isvip', 1, '='));
        if ($from_chapterorder) {
            $this->db->criteria->add(new Criteria('chapterorder', $from_chapterorder, '>='));
        }
		$this->db->criteria->add(new Criteria('display', 0, '='));
		...
		while($v = $this->db->getObject()){
		     if (!in_array($v->getVar('chapterid'),$buyArr)){
                 ...
                 if ($from_cid && $useregold_tmp < $salepriceAf) {
                     break;
                 }
                 else {
                     $useregold_tmp -= $salepriceAf;
                 }
...
/*		$i = 0;
    ...
    function updateChapterVisit($cid) {
        return $this->redis->hIncrBy("ChapterVisit",$cid,1);
    }
    ...
    function makeHtml($nowid, $show = false, $filter = false, $obj = NULL)
    {
        ...
        //非本频道书跳转到相应频道
        if (!in_array(JIEQI_MODULE_NAME, array('3gwap', 'wap', '3g', 'overseas')) && $show) {//echo $this->getModule($data['article']['sortid']);exit;
            ...
            if (defined('RETURN_READER_DATA') && RETURN_READER_DATA) {
                ...
                if ($data['chapter']['isvip'] && !defined('RETURN_READER_CONTENT') && isset($auth['uid']) && $auth['uid'] || $_SESSION['jieqiFreeAid'] == $this->id) {
                    if (!$data['chapter']['saleprice'] || $_SESSION['jieqiFreeAid'] == $this->id) define('RETURN_READER_CONTENT', true);
                        ...
                        if ($this->checkChapterIsBuy($chapterid)) {
...
//								}
                            if (!defined('RETURN_READER_CONTENT') || !RETURN_READER_CONTENT) {
                                ...
                                if (defined("PLATFORM") && PLATFORM!='mobile') { //手机上不需要运行
                                ...
                                $useshuquan = 1;  //in_array($data['chapter']['chapterid'], $tmpchapters) ? 1 : 0;//当前章节是否使用书券
...
// 		$data['chapter']['index_page'] = ($this->geturl('article', 'index', 'aid='.$this->id));//basename
        ...
        if ($preid > 0) {
            ...
            $data['chapter']['preview_page'] = "/read/".$this->id."/".$tmpvar.".html";
        ...
        if ($nextid > 0) {
            ...
            $data['chapter']['next_page'] = "/read/".$this->id."/".$tmpvar.".html";
        ...
        if ($data['chapter']['isvip'] > 0) {//如果是VIP章节，则显示另一套模板
            ...
            //2015-5-28 add chengyuan 海外阅读添加
            if (JIEQI_MODULE_NAME == 'overseas') {
                $data['chapter']['readvip'] = $this->geturl(JIEQI_MODULE_NAME, 'reader', 'method=readvip', 'aid=' . $this->id, 'cid=' . $chapterid, 'SYS=method=readvip');
            } else {
        ...
        if ($show) {
            //2015-5-30 add chengyuan
            //查询当月投票信息
            $arr = $this->getMonthData($this->id);
            $data['month_vipvote'] = $arr['vipvote'];
            $data['month_vote'] = $arr['vote'];
            $data['month_reward'] = $arr['reward'];
	...
	function makeIndex($show=false, $obj = NULL){
		 ...
		 $data['article'] = $this->formatOPF();
		 //非本频道书跳转到相应频道
		 if(!in_array(JIEQI_MODULE_NAME,array('3gwap','wap','3g','overseas')) && $show){
			 $p_tmp = parse_url(JIEQI_CURRENT_URL);
		 ...
		 if(JIEQI_MODULE_NAME=='overseas')$cols=2;//海外平台,特殊处理 add 2015-5-28 chengyuan
		 $indexrows=array();
		 $i=0;
		 $idx=0;
		 $this->preid=0; //前一章
	...
	function getStatArray(){
	     return array(
		      ...
		      'goodnum'=>array('name'=>'收藏','writelog'=>true,'format'=>'把作品《<a href="{$url_articleinfo}" target="_blank">{$articlename}</a>》放入了书架。'),
	     	   'autogoodnum'=>array('name'=>'自动收藏','writelog'=>false,'format'=>'把作品《<a href="{$url_articleinfo}" target="_blank">{$articlename}</a>》自动放入了书架。'),
...
// 						$article_handler->db->query($sql);*/
	 ...
	 * 查询本月月票、推荐票、打赏
	 * 用于海外版详情页、阅读页
	 * @author chengyuan 2015-5-30 下午12:07:56
	 * @param unknown $aid
	 * @return unknown
	 */
	function getMonthData($aid){
		...
		$data['vote'] = $this->db->getSum('stat');//本月推荐票
		...
		$this->db->criteria->add(new Criteria( 'addtime', $monthstart, '>=' ));
		$data['vipvote'] = $this->db->getSum('stat');//本月月票
		...
		$this->db->criteria->add(new Criteria( 'addtime', $monthstart, '>=' ));
		$data['reward'] = $this->db->getSum('stat');//本月打赏
	 ...
	 * 海外版：查询详情页其他信息
	 */
	function getOhterData($aid){
		$data['dayvisit'] = $data['$monthvisit'] = 0;
		...
		$this->db->queryObjects();
		while($v = $this->db->getObject()){
			$lasttime = $v->getVar('lasttime');
			if($lasttime >= $this->getTime()) $data['dayvisit'] = $v->getVar('day');
			if($lasttime >= $this->getTime('month')) $data['monthvisit'] = $v->getVar('month');
		}
	
		$this->db->init('sale','saleid','article');
		$this->db->setCriteria();
		$this->db->criteria->add(new Criteria('articleid',$aid));
		$data['salenum'] = $this->db->getCount();
		...
		$this->db->criteria->add(new Criteria('display',0));
		$this->db->criteria->add(new Criteria('isvip',1));
		$data['paynum'] = $this->db->getCount();
		$data['totalprice'] = $this->db->getSum('saleprice');
	
		return $data;
	}

    function write_cache_php() {
        ...
        for($i=0;$i<count($chapter_list);$i++) {
            ...
            $x=explode('.',$c['href']);
            $chapterid=$x[0];
            $chapter_list[$i]['chapterid']=$chapterid;
            $chapter_idx[$chapterid]=$i;
        }

        $article_data['chapter_list']=$chapter_list;
        $article_data['chapter_idx']=$chapter_idx;

        $s = var_export($article_data,true);
        $cache_php="<?php\n\$article_data=$s;\n";

        $opfdir=$this->getDir('opfdir');
        jieqi_writefile($opfdir.'/index.php', $cache_php);
    }

	public function checkArticleReader($aid){
		$this->addConfig('article','power');
		$jieqiPower['article'] = $this->getConfig('article','power');
		if($this->checkpower($jieqiPower['article']['freeread'], $this->getUsersStatus (), $this->getUsersGroup (), true)) return true;
		$auth = $this->getAuth();

...
//		 if($this->submitcheck()){
             ...
             if(isset($params['siteid'])){
			 	$this->db->criteria->add(new Criteria('siteid', intval($params['siteid'])));
			 }
			 //状态查询条件
             if(is_numeric($params['fullflag'])){
             	if($params['fullflag'] == 2){
             		$this->db->criteria->add(new Criteria('fullflag', 0));
             	}else{
             		$this->db->criteria->add(new Criteria('fullflag', $params['fullflag']));
             	}
			 }
//			 if($params['siteid'] != -1){
		 ...
		 if($params['action']=='downexcel'){
		      ...
		      switch($params['action']){
				   ...
				   case 'novip'://取消VIP
					   ...
					   $this->db->updatetable('article_chapter', array('saleprice'=>0,'isvip'=>0),array('articleid'=>$params['id']));
					   $package->article_repack($params['id'], array('makeopf'=>1), 1);
				   break;

...
/**
 * 传媒公司专用
 * @author chengyuan 2015-5-12 上午11:01:57
 */
...
// 		 $this->db->setCriteria(new Criteria('a.siteid', JIEQI_SITE_ID, '='));
		 $this->db->criteria->setTables($this->dbprefix('article_article')." as a INNER JOIN ".$this->dbprefix('pooling_article')." AS pa on a.articleid=pa.articleid LEFT JOIN ".$this->dbprefix('article_statamout')." AS s ON a.articleid=s.articleid");
		 $this->db->criteria->setFields("a.*,s.visit,s.vote,s.goodnum,s.vipvote,s.reward");
		 $this->db->criteria->add(new Criteria('pa.channelid', '62'));
		 //提交数据
//		 if($this->submitcheck()){
			 if(!empty($params['keyword'])){
				if($params['keytype']==1) $this->db->criteria->add(new Criteria('a.author', $params['keyword'], '='));
				elseif($params['keytype']==2) $this->db->criteria->add(new Criteria('a.poster', $params['keyword'], '='));
				else $this->db->criteria->add(new Criteria('a.articlename', '%'.$params['keyword'].'%', 'LIKE'));
			 }
			 ...
			 if(is_numeric($params['firstflag'])){
			 	$this->db->criteria->add(new Criteria('a.firstflag', $params['firstflag']));
			 }
             // 添加频道查询条件 by snowdiva
             if(isset($params['siteid'])){
// 			 	$this->db->criteria->add(new Criteria('pa.siteid', intval($params['siteid'])));
			 }
			 //状态查询条件
             if(is_numeric($params['fullflag'])){
             	if($params['fullflag'] == 2){
             		$this->db->criteria->add(new Criteria('a.fullflag', 0));
             	}else{
             		$this->db->criteria->add(new Criteria('a.fullflag', $params['fullflag']));
             	}
			 }
//			 if($params['siteid'] != -1){
...
//		 }
		 if(!empty($params['display'])){
			 switch ($params['display']){
				...
				case 'show':
					...
					break;
				case 'sign':
					$this->db->criteria->add(new Criteria('a.permission', 4, '>='));
				...
				case 'empty':
					...
					break;
				case 'cool':
					$this->db->criteria->add(new Criteria('a.postdate', time()-3600*24*30, '<'));
		...
		$data = $package->getSources();           // 添加文章及频道分类 by snowdiva
...
// 		echo $this->db->returnSql($this->db->criteria);
	...
	public function action($params){
		 if($params['action']=='downexcel'){
		 	...
		 	$this->db->criteria->setTables($this->dbprefix('article_article')." as a INNER JOIN ".$this->dbprefix('pooling_article')." AS pa on a.articleid=pa.articleid LEFT JOIN ".$this->dbprefix('article_statamout')." AS s ON a.articleid=s.articleid");
// 		 	$this->db->criteria->setFields("a.*,s.visit,s.vote,s.goodnum,s.vipvote,s.reward");
		 	$this->db->criteria->setFields("a.*,s.visit,s.vote,s.goodnum,s.vipvote,s.reward,s.sale,(select count(*) from ".$this->dbprefix('article_chapter')." where articleid=a.articleid and chaptertype=0 and isvip=0) as freechapters,pa.adddate");
		 	$this->db->criteria->add(new Criteria('pa.channelid', '62'));
...
// 			  if(!$params['all']) $this->db->criteria->add(new Criteria('permission', 4, '>='));
			  $this->db->queryObjects($this->db->criteria);
			  ...
			  while($v = $this->db->getObject()){
				   ...
				   $data[$k] = array(
						  ...
						  'signdate'=>$articlerows[$k]['signdate']>0?date('Y-m-d H:i:s', $articlerows[$k]['signdate']):'未签约',
						  ...
						  'url_articleinfo'=>$articlerows[$k]['url_articleinfo'],
				   		'adddate'=>date('Y-m-d H:i:s', $articlerows[$k]['adddate']),
// 						  'sale'=>$articlerows[$k]['sale']
		      ...
		      switch($params['action']){
				   ...
				   break;
				   case 'resign'://重设签约时间
				   		$signdate = strtotime($params['signdate']);
				       $query = $this->db->edit($params['id'],array('signdate'=>$signdate));
					   $package->article_repack($params['id'], array('makeopf'=>1), 1);
				   break;
			       case 'fullflag'://设置完本
...
//			$articleLib->delPower($deleteid);
	...
	//设置vip
	public function setvip($params = array()){
	     global $jieqiModules;
	     $this->addConfig('article','configs');
	     $jieqiConfigs['article'] = $this->getConfig('article','configs');
		if(!empty($params['aid'])){
			...
			while ( $chapter = $this->db->getObject () ) {
				$data['articlename'] = $chapter->getVar ( 'articlename' );
				...
				$chapterrows [$k] ['saleprice'] = $chapter->getVar ( 'saleprice' );
				$k ++;
			}
			$data['chapterrows'] = $chapterrows;
			$data['aid'] = $params['aid'];
			$data['article_static_url'] = (empty ( $jieqiConfigs ['article'] ['staticurl'] )) ? $jieqiModules ['article'] ['url'] : $jieqiConfigs ['article'] ['staticurl'];
			...
			$this->db->init('chapter','chapterid','article');
			$this->db->setCriteria(new Criteria('chapterid',$params['cid']));
			$chapterObj = $this->db->get($this->db->criteria);
			$chapterorder = $chapterObj->getVar('chapterorder');
			$articleid = $chapterObj->getVar('articleid');
			
			$this->db->init('article','articleid','article');
			$article = $this->db->get($articleid);
			if($article['articletype']==0){
				if(!$this->db->edit($articleid,array('articletype'=>1, 'vipdate'=>JIEQI_NOW_TIME))) $this->printfail(LANG_DO_FAILURE);
			}
			$this->db->init('chapter','chapterid','article');
			$this->db->setCriteria ( new Criteria ( 'articleid', $articleid));
			$this->db->criteria->add(new Criteria('chapterorder', $chapterorder, '>='));
			$this->db->criteria->add(new Criteria('chaptertype', 0));
			...
			$package = $this->load('article','article');//加载文章处理类
			while ( $chapter = $this->db->getObject () ) {
				$chaptersize = $chapter->getVar ( 'size' );
				$chapterid = $chapter->getVar ( 'chapterid' );
				...
				if(!empty($chaptersize)){
					...
					if(is_numeric($jieqiConfigs['article']['wordsperegold']) && $jieqiConfigs['article']['wordsperegold']>0){
						...
						if($jieqiConfigs['article']['priceround']==1){
							$saleprice=floor($chaptersize / $wordsperegold);//向下舍入，取整。
						}elseif($jieqiConfigs['article']['priceround']==2){
							$saleprice=ceil($chaptersize / $wordsperegold);//向上舍入，取整。
						}else{
							$saleprice=round($chaptersize / $wordsperegold);//四舍五入
						}
					}
					
					
				}
				$this->db->edit($chapterid,array('isvip'=>1,'saleprice'=>$saleprice));//,'lastupdate'=>JIEQI_NOW_TIME
				$k ++;
			}
			$package->article_repack($articleid, array('makeopf'=>1));
			$this->jumppage($jieqiModules ['article'] ['url'].'/admin/?controller=article&method=setvip&aid='.$articleid);

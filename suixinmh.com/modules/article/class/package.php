...
if(!function_exists('jieqi_get_block')) include_once(JIEQI_ROOT_PATH.'/header.php');
...
class JieqiPackage extends JieqiObject
{
	...
	//取得文件保存目录
	function getDir($dirtype='txtdir', $idasdir=true, $automake=true, $issubdir=true)
	{
		global $jieqiConfigs;
		if($dirtype=='htmldir'){
			$P = parse_url($this->geturl('article', 'index', 'aid='.$this->id));
			$retdir = HLM_ROOT_PATH.dirname($P['path']);
			if ($automake && !file_exists($retdir)) jieqi_createdir($retdir,0777,true);
		}else{
			//定义是否按ID分类生成文件夹
			if(isset($jieqiConfigs['article'][$dirtype.'sub'])) $issubdir = $jieqiConfigs['article'][$dirtype.'sub'];
			//定义是否按ID生成文件夹
			$isidasdir = $idasdir;
			if(isset($jieqiConfigs['article'][$dirtype.'idas'])) $isidasdir = $jieqiConfigs['article'][$dirtype.'idas'];
			
			$retdir=jieqi_uploadpath($jieqiConfigs['article'][$dirtype], 'article');
			
			if ($automake && !file_exists($retdir)){
				jieqi_createdir($retdir,0777,true);
			}
			if($issubdir){
	...
	function editPackage($infoary=array(), $save=true)
	{
		...
		$tmpstr=$this->metas['dc:Articlename'];
		foreach($infoary as $k=>$v){
			$this->metas['dc:'.ucfirst($k)]=$v;
		}
		$this->metas['dc:Date']=date(JIEQI_DATE_FORMAT);
	...
	function createOPF($save=true)
	{
		...
		$id = $this->getId();
        $this->redis->del("vip_chapter_list_".$id);
    ...
    function read_cacheOPF() {
        $id=$this->getId();
        $cache_data=$this->redis->get("cacheOPF_".$id);
        if ($cache_data) {
            $data_arr=unserialize($cache_data);
            if ($data_arr['metas'] && $data_arr['chapters']) {
                $this->metas=$data_arr['metas'];
                $this->chapters=$data_arr['chapters'];
                return true;
            }
        }
        return false;
    ...
    function write_cacheOPF() {
        $id=$this->getId();
        if ($this->metas && $this->chapters) {
            $cache_arr=array("metas"=>$this->metas,"chapters"=>$this->chapters);
            $cache_data=serialize($cache_arr);
            $this->redis->set("cacheOPF_".$id,$cache_data);
            return true;
        }
        return false;
    }
	...
	//显示一个章节
	function showChapter($cid,$isreturn = false)
	{
		global $jieqiConfigs;
		global $jieqi_file_postfix;
		$i=0;
		...
		while($i<$num){
			...
			if($tmpvar==$cid){
				$this->makeHtml($i+1, true, true, true,$isreturn);
				return true;
			}
			$i++;
		}
	...
	//生成一个章节的html
	function makeHtml($nowid, $dynamic=false, $show=false, $filter=false,$isreturn = false)
	{
		global $jieqiConfigs;
		global $jieqiSort;
		global $jieqiTpl;
		...
		$jieqiTpl->assign('article_title', jieqi_htmlstr($this->metas['dc:Articlename']));
		...
		$jieqiTpl->assign('authorid',intval($this->metas['dc:Authorid']));
		$jieqiTpl->assign('author', jieqi_htmlstr($this->metas['dc:Author']));
		...
		$jieqiTpl->assign('url_fullpage', jieqi_geturl('article', 'article', $this->id, 'fulldir'));
		//打包下载
		$jieqiTpl->assign('url_download', jieqi_geturl('article', 'article', $this->id, 'zipdir'));
		//自己页面
		...
		//书目页
        $jieqiTpl->assign('url_indexpage', jieqi_geturl('article', 'article', $this->id, 'index'));
		$jieqiTpl->assign('postdate',$this->chapters[$nowid-1]['postdate']);
		$jieqiTpl->assign('lastupdate',$this->chapters[$nowid-1]['lastupdate']);
		$jieqiTpl->assign('size',$this->chapters[$nowid-1]['size']);
		...
		//网址改成可以点击的
		if($isreturn!=2) $tmpvar=$ts->makeClickable(jieqi_htmlstr($tmpvar));
		//加入文字水印
...
'.$textwatermark.'<br />'.$v;
		...
		if($preid>0){
			...
			$jieqiTpl->assign('pre_chapterid',$tmpvar);
		}else{
	...
	function makeIndex($dynamic=false, $show=false)
	{
		...
		//生成index.html
		$articlename=jieqi_htmlstr($this->metas['dc:Articlename']);
		//文章序号及名称
		$jieqiTpl->assign('dynamic_url', ARTICLE_DYNAMIC_URL);
		$jieqiTpl->assign('static_url', ARTICLE_STATIC_URL);
		...
		$jieqiTpl->assign('size', intval($this->metas['dc:Size']));
		$articletype=intval($this->metas['dc:Articletype']);
		$jieqiTpl->assign('articletype', $articletype);
		if(($articletype & 1)>0) $jieqiTpl->assign('hasebook', 1);
		else $jieqiTpl->assign('hasebook', 0);
	...
	function makezip(){
		...
		if (@function_exists('gzcompress')){
            ...
            $P = parse_url($this->geturl('article', 'index', 'aid='.$this->id));
			$dir = HLM_ROOT_PATH.dirname($P['path']);
			//逐个加文件，因为里面有内容需要替换
			$filelist=array();
			if (file_exists($dir)){
				$dh = opendir($dir);
			...
			if (count($filelist)>0){
				...
				$zip->setComment("Powered by HLM CMS");
	...
	function maketxtfull(){
		...
		$data .= '<'.$this->metas['dc:Articlename'].'>'.$br;
		$volume='';
		foreach($this->chapters as $k => $chapter){
			if($chapter['content-type']=='volume'){
				$volume=$chapter['id'];
	...
	function makeumd_volume($vk = 0){
		...
		if(empty($vk) || $vk < $vd){
			...
			$umd->setinfo(array('id'=>$this->id, 'title'=>$this->metas['dc:Articlename'], 'author'=>$this->metas['dc:Author'], 'sort'=>$sort, 'publisher'=>$this->metas['dc:Publisher'], 'corver'=>'')); //设置文章信息
			...
			foreach($this->chapters as $k => $chapter){
				if($chapter['content-type']=='volume'){
					...
					if ($vnew) {
						...
						$umd->setinfo(array('id'=>$this->id, 'title'=>$this->metas['dc:Articlename'].'_'.$vk.'_'.$vid, 'author'=>$this->metas['dc:Author'], 'sort'=>$sort, 'publisher'=>$this->metas['dc:Publisher'], 'corver'=>'')); //设置文章信息
	...
	function makejar_volume($vk = 0){
		...
		if(empty($vk) || $vk < $vd){
			...
			$jar->setinfo(array('id'=>$this->id, 'title'=>$this->metas['dc:Articlename'], 'author'=>$this->metas['dc:Author'], 'publisher'=>$this->metas['dc:Publisher'], 'corver'=>'')); //设置文章信息
			...
			foreach($this->chapters as $k => $chapter){
				if($chapter['content-type']=='volume'){
					...
					if ($vnew) {
						$jar=new JieqiJar();
						$jar->setcharset(strtoupper(JIEQI_SYSTEM_CHARSET));
						$jar->setinfo(array('id'=>$this->id, 'title'=>$this->metas['dc:Articlename'].'_'.$vk.'_'.$vid, 'author'=>$this->metas['dc:Author'], 'publisher'=>$this->metas['dc:Publisher'], 'corver'=>'')); //设置文章信息
						$vnew = false;
						$vinfo[$vid]['chapters'] = 0;
						$vinfo[$vid]['volumes'] = 0;
						$vinfo[$vid]['fromvolume'] = $volume;
	...
	function makepack(){
		...
		$url=$this->geturl ( 'article', 'article', 'SYS=method=synchronousMakePack&key='.urlencode(md5(JIEQI_DB_USER.JIEQI_DB_PASS.JIEQI_DB_NAME.JIEQI_SITE_KEY)).'&aid='.intval($this->id));
		$url=trim($url);
...
// 		$size = 'size';
		...
		if($type) $contenttype='volume';
			...
			jieqi_writefile($txtdir.'/'.$chapterid.$jieqi_file_postfix['txt'], $content);//生成章节文件
		} 
		if($display) return true;
		/*if(!$this->isload) $this->loadOPF($articleid);
		$articlename=jieqi_htmlstr($this->metas['dc:Articlename']);
		$chaptercount=count($this->chapters);
		//处理新章节的位置
		if($volumeid>0){
...
//         $display = !$display ? 0 : 2;
		if($volumeid>$chaptercount){
			//追加章节
// 			$this->chapters[]=array('id'=>$name, 'href'=>$chapterid.$jieqi_file_postfix['txt'], 'media-type'=>'text/html', 'content-type'=>$contenttype,'display'=>$display,'postdate'=>$postdate,'lastupdate'=>$lastupdate,'size'=>$size);
			$this->chapters [] = array (
					'id' => str_replace(' ','', $name ),
					'href' => $chapterid.$jieqi_file_postfix['txt'],
...
// 			$this->chapters[$volumeid-1]=array('id'=>$name, 'href'=>$chapterid.$jieqi_file_postfix['txt'], 'media-type'=>'text/html', 'content-type'=>$contenttype,'display'=>$display,'postdate'=>$postdate,'lastupdate'=>$lastupdate,'size'=>$size);
			$this->chapters [$volumeid-1] = array (
					'id' => str_replace(' ','', $name ),
					'href' => $chapterid.$jieqi_file_postfix['txt'],
					'media-type' => 'text/html',
					'content-type' => $contenttype,
		...
		$this->addConfig('article','url');
		$setindex = $this->getConfig('article','url','index_main');
		if(isset($setindex['ishtml']) && $setindex['ishtml']){
			//生成html目录
			$this->nowid=$volumeid;
			$this->makeIndex();
			$this->preid = $this->nowid-1;
			$this->nextid = $this->nowid+1;
		}
		...
		if($jieqiConfigs['article']['makeopf']) $ptypes['makeopf'] = 1;
		if($jieqiConfigs['article']['makehtml']) $ptypes['makehtml'] = 1;
		if($jieqiConfigs['article']['makezip']) $ptypes['makezip'] = 1;
		if($jieqiConfigs['article']['makefull']) $ptypes['makefull'] = 1;
		if($jieqiConfigs['article']['maketxtfull']) $ptypes['maketxtfull'] = 1;
		if($jieqiConfigs['article']['makeumd']) $ptypes['makeumd'] = 1;
		if($jieqiConfigs['article']['makejar']) $ptypes['makejar'] = 1;
		article_repack($this->id, $ptypes);	*/		
	}

	//编辑章节
	function editChapter( $chapterid, $chapterorder, &$content, $chapterarray = array())//name,&$content,$type,$display, $chapterorder, $chapterid,$postdate,$lastupdate,$size,$intro
	{
		global $jieqiConfigs;
		global $jieqi_file_postfix;
		$txtdir=$this->getDir('txtdir');
		jieqi_writefile($txtdir.'/'.$chapterid.$jieqi_file_postfix['txt'], $content);
		if($chapterarray['display']) return true;
		$this->loadOPF();
		$articlename=jieqi_htmlstr($this->metas['dc:Articlename']);
		$contenttype=$chapterarray['content-type'];
		//$display=$this->chapters[$chapterorder-1]['display'];
		$this->chapters[$chapterorder-1]=$chapterarray;
		//array('id'=>$name, 'href'=>$chapterid.$jieqi_file_postfix['txt'], 'media-type'=>'text/html', 'content-type'=>$contenttype,'display'=>$display,'postdate'=>$postdate,'lastupdate'=>$lastupdate,'size'=>$size,'intro'=>$intro);
		$this->createOPF();
		//生成html
		...
		if($contenttype=='chapter'){
		    //章节
			$setreader = $this->getConfig('article','url','reader_main');
		    if(isset($setreader['ishtml']) && $setreader['ishtml']) $this->makeHtml($this->nowid);
		}
	...
	//删除章节
	function delChapter($chapterorder, $chapterid, $display=0)
	{ 
		global $jieqiConfigs;
		global $jieqi_file_postfix;
		$txtdir=$this->getDir('txtdir', true, false);
       ...
       */
		...
		$this->addConfig('article','url');
		$setreader = $this->getConfig('article','url','reader_main');
		if(isset($setreader['ishtml']) && $setreader['ishtml']){//if($jieqiConfigs['article']['makehtml']){
			//生成html目录
	...
	function sortChapter($fromid, $toid)
	{
		...
		$this->addConfig('article','url');
		$setindex = $this->getConfig('article','url','index_main');
		if($setindex['ishtml']  && $setindex['ishtml']){
			//生成html目录
			$this->makeIndex();
		}
		//章节调换顺序需要重新生成html
	...
	function delete()
	{
		...
		$this->addConfig('article','url');
		$setreader = $this->getConfig('article','url','reader_main');
		if(isset($setreader['ishtml']) && $setreader['ishtml']) jieqi_delfolder($this->getDir('htmldir', true, false));
	...
	function getContent($id){
		...
		return jieqi_readfile($this->getDir('txtdir', true, false).'/'.intval($id).$jieqi_file_postfix['txt']);
	}

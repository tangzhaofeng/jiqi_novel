...
//				if (JIEQI_MODULE_NAME != 'wap' && JIEQI_MODULE_NAME != '3gwap'){
...
//			}
		...
		if(!in_array($params['type'],array('monthsize','weeksize','daysize'))){//如果是字数榜
		 ...
		 $maxpage = ceil($this->db->getCount($this->criteria)/$jieqiConfigs['article']['toppagenum']);
		 $k=0;
		...
		//print_r($data['channel'] );exit;
		 if (isset($params['siteid'])){
			...
			$pageurl =  $this->geturl($module,'top','method=toplist','evalpage=0','SYS=type='.$params['type'].'&sortid='.$params['sortid'].'&page='.$params['page'].'&siteid='.$siteid);
			$wappageurl =  $this->geturl($module,'top','method=toplist','SYS=type='.$params['type'].'&sortid='.$params['sortid'].'&page='.$params['page'].'&siteid='.$siteid);
		 }else{

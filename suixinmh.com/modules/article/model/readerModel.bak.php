...
/*		if(!$chapter->getVar('saleprice')){
		...
		$osale['saleprice'] = $datavip['chapter'][0]['shubi'] ? $datavip['chapter'][0]['shubi'] : $datavip['chapter'][0]['shuquan'];
		...
		if(!defined('NOT_PUT_DATA')){
			if($retstatu) $this->jumppage($readurl, LANG_DO_SUCCESS, $jieqiLang['article']['batch_buy_success']);
	...
	public function batchbuychapter($params)
	{
		...
		$useregold=$users->getVar('egold', 'n');//账号余额
		$useresilver=$users->getVar('esilver', 'n');//书券余额
		$datavip = $package->getAllVip($params['aid'],$params['cids']);
		if(!$datavip['num']) $this->printfail(LANG_ERROR_PARAMETER);
		//$this->printfail($datavip['num']);
		$articlename = $article->getVar('articlename');
		$readurl = $this->geturl('article', 'reader', 'aid='.$params['aid'],'cid='.$datavip['chapter'][0]['chapterid']);
		//验证金额是否足够
		$moneyEnough = true;
        if($datavip['shuquan']){
		     if($useresilver < $datavip['shuquan']) $moneyEnough = false;
		...
		foreach ($datavip['chapter'] as  $k=>$v){
			...
			$osale['articleid'] = $article->getVar('articleid');
			$osale['saleprice'] = $datavip['chapter'][$k]['shubi'] ? $datavip['chapter'][$k]['shubi'] : $datavip['chapter'][$k]['shuquan'];
